...
function acceptedVariants(number: string) {
  const out = new Set([number]);
  if (number.length === 13 && number.startsWith('55') && number.charAt(4) === '9') {
    out.add(`${number.slice(0, 4)}${number.slice(5)}`);
  }
  if (number.length === 12 && number.startsWith('55')) {
    out.add(`${number.slice(0, 4)}9${number.slice(4)}`);
  }
  return out;
}
...
async function getState() {
  ...
  try {
    ...
    return {
      ok: response.ok,
      state: String(payload?.instance?.state || payload?.state || 'unknown').toLowerCase(),
...
function buildPdf() {
  ...
  let padding = '';
  while (Buffer.byteLength(padding, 'utf8') < 1_020_000) padding += padLine;
  body += padding;
...
export async function GET(request: NextRequest) {
  ...
  if (!before.ok || before.state !== 'open') {
    return NextResponse.json({ ok: false, before }, { status: 409 });
  }
  ...
  try {
    ...
    const key = payload?.key || payload?.response?.key || payload?.response?.message?.key || payload?.data?.key || {};
    const status = String(
      payload?.status || payload?.response?.status || payload?.response?.message?.status || payload?.data?.status || 'UNKNOWN',
    ).toUpperCase();
    const id = String(key?.id || '');
    const remoteJid = String(key?.remoteJid || '');
    const destinationOk =
      !remoteJid || [...acceptedVariants(TEST_NUMBER)].some((n) => `${n}@s.whatsapp.net` === remoteJid);
    const ok = response.ok && Boolean(id) && destinationOk && !['ERROR', 'FAILED', 'CANCELED', 'CANCELLED'].includes(status);
