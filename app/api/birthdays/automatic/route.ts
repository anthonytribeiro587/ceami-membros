...
export async function POST() {
  ...
  if (setting.test_mode && !setting.test_member_id) {
    return NextResponse.json(
      { error: 'Selecione o membro de teste em Ajustes.' },
      { status: 400 },
    );
  }
  ...
  const accepted = result.ok && payload.ok === true && payload.accepted !== false;
  const deliveryStatus = accepted
    ? String(payload.deliveryStatus || 'queued')
    : 'failed';
  const errorMessage = accepted
    ? null
    : String(payload.error || payload.details || 'Falha no envio');

  await supabase
    .from('birthday_automation_settings')
    .update({
      last_sent_date: accepted ? clock.date : null,
